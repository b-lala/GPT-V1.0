...
def main():
    import gradio as gr
#    if gr.__version__ not in ['4.9.1']: 
#        raise ModuleNotFoundError("使用项目内置Gradio获取最优体验! 请运行 `pip install -r requirements.txt` 指令安装内置Gradio及其他依赖, 详情信息见requirements.txt.")
    ...
    customize_btns = {}
    predefined_btns = {}
    with gr.Blocks(title="GPT", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        gr.HTML(title_html)
...
#        blocked_paths=["config.py","config_private.py","docker-compose.yml","Dockerfile",f"{PATH_LOGGING}/admin"])
    ...
    # 如果需要在二级路径下运行
   # CUSTOM_PATH = get_conf('/root/cht/gpt_academic')
   # if CUSTOM_PATH != "/": 
   #     from toolbox import run_gradio_in_subpath
   #     run_gradio_in_subpath(demo, auth=AUTHENTICATION, port=PORT, custom_path=CUSTOM_PATH)
   # else: 
   #     demo.launch(server_name="0.0.0.0", share=True, server_port=PORT, auth=AUTHENTICATION, favicon_path="docs/logo.png",
   #                 blocked_paths=["config.py","config_private.py","docker-compose.yml","Dockerfile",f"{PATH_LOGGING}/admin"])
